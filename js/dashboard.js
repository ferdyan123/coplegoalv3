...
function renderHomePage() {
  ...
  const namesEl = $('home-couple-names');
  if (namesEl && s.name1 && s.name2) namesEl.textContent = s.name1 + ' & ' + s.name2;
  const p1Title = $('home-assignee-title-p1');
  const p2Title = $('home-assignee-title-p2');
  const p1Icon  = $('home-assignee-icon-p1');
  const p2Icon  = $('home-assignee-icon-p2');
...
function renderDashboard() {
  ...
  if (expenseActual > expenseBudget && expenseBudget > 0) {
    ep.textContent = 'OVER BUDGET ⚠️'; ep.className = 'summary-pill over';
  } else {
    ep.textContent = 'ON BUDGET ✅';   ep.className = 'summary-pill';
  }
...
function getFinanceStickerConfig() {
  ...
  if (totalTx === 0 && incomeBudget === 0) return { file: STICKER_FILES.nodata,    cls: 'sticker--idle',    title: 'Belum ada data nih~',      desc: 'Yuk mulai catat keuangan kalian! 🐾' };
  if (goalDone)                            return { file: STICKER_FILES.celebrate, cls: 'sticker--celebrate',title: 'Goal tercapai! 🎉',          desc: 'Kalian luar biasa — target tabungan kelar!' };
  if (surplusPct >= 0.30 && income > 0)   return { file: STICKER_FILES.happy,     cls: '',                  title: 'Keuangan sehat banget!',     desc: `Sisa ${Math.round(surplusPct*100)}% dari pemasukan 🌟` };
  if (surplusPct >= 0.10 && income > 0)   return { file: STICKER_FILES.ontrack,   cls: '',                  title: 'On track, good job!',        desc: 'Pengeluaran terkendali ✅' };
  if (savActual > 0 && expRatio < 0.80)   return { file: STICKER_FILES.saving,    cls: '',                  title: 'Rajin nabung nih!',          desc: 'Konsisten = kunci kebebasan finansial 🐟' };
  if (totalTx === 0 && incomeBudget > 0)  return { file: STICKER_FILES.welcome,   cls: 'sticker--idle',     title: 'Siap pantau keuangan~',      desc: 'Anggaran sudah diset, yuk catat transaksi! 👀' };
  if (expRatio >= 0.80 && expRatio < 1.0) return { file: STICKER_FILES.warning,   cls: '',                  title: 'Hati-hati nih...',           desc: `Pengeluaran udah ${Math.round(expRatio*100)}% dari budget ⚠️` };
  if (expenseActual > expenseBudget && expenseBudget > 0) return { file: STICKER_FILES.crisis, cls: 'sticker--crisis', title: 'Aduh, over budget! 😭', desc: 'Pengeluaran melebihi anggaran — yuk evaluasi!' };
  if (expRatio >= 0.70)                   return { file: STICKER_FILES.tired,     cls: '',                  title: 'Pengeluaran lumayan nih',    desc: 'Kurangi sedikit biar lebih lega~' };
  return { file: STICKER_FILES.happy, cls: '', title: 'Keuangan aman!', desc: 'Terus pertahankan ya 💪' };
}

function renderFinanceSticker() {
  const el = $('finance-sticker-area'); if (!el) return;
...
function calcHealthScore() {
  ...
  if (incomeBudget === 0 && incomeActual === 0)
    return { score: 0, grade: 'Belum Ada Data', tips: [], color: '#5a6690' };
...
function renderDonutChart() {
  ...
  const data   = CATEGORIES.map(c => calcActual(c.key));
  const colors = getThemeColors();
  const total  = data.reduce((s, v) => s + v, 0);

  const totalEl = $('donut-budget-total');
  if (totalEl) totalEl.textContent = fmt(total);
  const legendEl = $('donut-legend');
  if (legendEl) legendEl.innerHTML = CATEGORIES.map((c, i) => `
  ...
  donutChart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [{ data, backgroundColor: colors, borderWidth: 3,
        borderColor: getCSSVar('--bg-surface') || 'transparent', hoverOffset: 8 }],
    },
    options: {
...
function renderBarChart() {
  const ctx = $('bar-overview') || $('bar-budget'); if (!ctx) return;
  const labels  = CATEGORIES.map(c => c.label.split(' ')[0]);
  const budgets = CATEGORIES.map(c => calcBudget(c.key));
  const actuals = CATEGORIES.map(c => calcActual(c.key));
  ...
  if (budgets.reduce((a,b)=>a+b,0) === 0 && actuals.reduce((a,b)=>a+b,0) === 0) return;
  barChart = new Chart(ctx, {
    ...
    data: { labels, datasets: [
      ...
      { label: 'Aktual', data: actuals,
        backgroundColor: accent2 + '99', borderColor: accent2,
        borderWidth: 2, borderRadius: 8, borderSkipped: false },
    ]},
    options: {
      responsive: true, maintainAspectRatio: true,
      plugins: {
        legend: { labels: { color: axisC, font: { size: 11 }, boxWidth: 12, borderRadius: 4 } },
        tooltip: {
      ...
      scales: {
        ...
        y: { ticks: { color: axisC, callback: v => fmt(v) }, grid: { color: gridC }, border: { color: 'transparent' } },
      },
    },
  });
}
...
function renderProgressCircle() {
  ...
  const circle = $('prog-fill-circle') || $('month-ring-fill');
  setTimeout(() => { if (circle) circle.style.strokeDashoffset = offset; }, 100);
  const pctEl = $('month-prog-pct') || $('month-progress-pct');
  if (pctEl) pctEl.textContent = ms.pct + '%';
  const lblEl = $('month-progress-label');
  if (lblEl) lblEl.textContent = `Hari ke-${ms.passed} dari ${ms.total}`;
  const passedEl = $('stat-days-passed'); if (passedEl) passedEl.textContent = ms.passed;
  const leftEl   = $('stat-days-left');   if (leftEl)   leftEl.textContent   = ms.left;
  const totalEl  = $('stat-days-total');  if (totalEl)  totalEl.textContent  = ms.total;

  /* SVG gradient — rebuild tiap ganti tema */
  ...
  if (svg) {
    ...
    if (circle) circle.setAttribute('stroke', 'url(#prog-gradient)');
  }
}
...
function renderRecentTransactions() {
  ...
  $('recent-tx-list').innerHTML = txs.map(t => renderTxItem(t)).join('');
}

/* ── TX ITEM renderer (shared) ── */
function renderTxItem(t) {
  const cat    = CATEGORIES.find(c => c.key === t.type) || { icon: '💰', color: '#8b5cf6', label: t.type };
  const s      = state.settings;
  ...
  return `<div class="tx-item" data-id="${t.id}">
    ...
    <div class="tx-body">
      ...
      <div class="tx-meta">${fmtDate(t.date)} · ${t.category || cat.label} · ${name}</div>
    </div>
...
function renderMonthlyReport() {
  ...
  trendChart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: 'Pemasukan',   data: incomeData,  borderColor: getCSSVar('--accent'),  backgroundColor: getCSSVar('--accent-soft')    || 'rgba(52,211,153,0.1)', tension: 0.4, fill: true, pointRadius: 4 },
        { label: 'Pengeluaran', data: expenseData, borderColor: getCSSVar('--accent-2'), backgroundColor: getCSSVar('--accent-2-soft') || 'rgba(248,113,113,0.1)', tension: 0.4, fill: true, pointRadius: 4 },
